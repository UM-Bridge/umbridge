...
def serve_models(models, port=4242, max_workers=1, error_checks=True):
    ...
    async def applyhessian(request):
        ...
        if error_checks:
            ...
            # Check if output dimension matches model output size outWrt
            if len(output) != input_sizes[in_wrt1]:
                return error_response("InvalidOutput", f"Output vector has invalid length! Model declared {input_sizes[in_wrt1]} but returned {len(output)}.", 500)
